...
ipcMain.on("select-directory", (event, certType) => {
  dialog
      ...
      if (!result.canceled && result.filePaths.length > 0) {
        ...
        if (students.length > 0) {
          ...
          generarPDF(
            ...
            certType,
            pdfContentGlobal
          ); // Llamamos a la función para generar los PDFs
        } else {
          console.error(
            "Error: No se pueden generar los PDFs. Asegúrate de cargar los datos primero."
          );
        }
      }
    })
    .catch((err) => {
      console.error(
        "Error al mostrar el diálogo de selección de directorio:",
        err
      );
    });
});
